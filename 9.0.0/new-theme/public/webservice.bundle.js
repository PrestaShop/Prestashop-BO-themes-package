 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:Et}=window;class Bt{constructor(){Et('input[id^="webservice_key_permissions_all"]').on("change",(t=>{const e=Et(t.currentTarget),n=e.is(":checked");e.closest("tr").find(`input:not(input[id="${e.attr("id")}"])`).each(((t,e)=>{Et(e).prop("checked",n)}))}))}}
/**
 * Copyright since 2007 PrestaShop SA and Contributors
 * PrestaShop is an International Registered Trademark & Property of PrestaShop SA
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:xt}=window;class Ot{constructor(t){this.onClick=t}extend(t){this.initRowLinks(t),this.initConfirmableActions(t)}initConfirmableActions(t){t.getContainer().on("click",e.linkRowAction,(t=>{const e=xt(t.currentTarget).data("confirm-message");e.length&&!window.confirm(e)&&t.preventDefault()}))}initRowLinks(t){const n=this.onClick;xt("tr",t.getContainer()).each((function(){const t=xt(this);xt(e.linkRowActionClickableFirst,t).each((function(){const o=xt(this),i=o.closest("td"),s=xt(e.clickableTd,t).not(i);let r=!1;s.addClass("cursor-pointer").on("mousedown",(()=>{xt(window).on("mousemove",(()=>{r=!0,xt(window).off("mousemove")}))})),s.on("mouseup",(()=>{const t=r;if(r=!1,xt(window).off("mousemove"),!t){const t=o.data("confirm-message");(!t.length||window.confirm(t)&&o.attr("href"))&&(D(n)||D(o.get(0))?document.location.href=o.attr("href"):n(o.get(0)))}}))}))}))}}
/**
 * Copyright since 2007 PrestaShop SA and Contributors
 * PrestaShop is an International Registered Trademark & Property of PrestaShop SA
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:jt}=window;jt((()=>{const t=new v("webservice_key");t.addExtension(new E),t.addExtension(new x),t.addExtension(new C),t.addExtension(new Ct),t.addExtension(new vt),t.addExtension(new pt),t.addExtension(new yt),t.addExtension(new j),t.addExtension(new Ot),new window.prestashop.component.ChoiceTree("#webservice_key_shop_association").enableAutoCheckChildren(),window.prestashop.component.initComponents(["MultipleChoiceTable","GeneratableInput"]),window.prestashop.instance.generatableInput.attachOn(".js-generator-btn"),new Bt,window.prestashop.component.initComponents(["MultistoreConfigField"])}))})(),window.webservice=o})();